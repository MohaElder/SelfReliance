...
namespace HeurekaGames.AssetHunter
{
    ...
    {
        [SerializeField]
        private List<AssemblyInfo> m_includedDependencies = new List<AssemblyInfo>();
        [SerializeField]
        public List<BuildReportAsset> m_BuildSizeList = new List<BuildReportAsset>();

        internal void AddDependency(string assemblyDependency)
        {
            var assemblies = AppDomain.CurrentDomain.GetAssemblies();

            foreach (var item in assemblies)
            {
                if (!assemblyDependency.Equals(item.ManifestModule.Name))
                    continue;

                m_includedDependencies.Add(new AssemblyInfo(item));
            }
        ...
        public List<AssemblyInfo> IncludedDependencies
        {
    ...
    splash.Add(UnityEditor.PlayerSettings.xboxSplashScreen);
        ...
        {
            m_includedDependencies.Sort((a, b) => b.size.CompareTo(a.size));
